...
export const cases = pgTable('cases', {
  ...
  judicialSystem: text('judicial_system'),
  judicialSystemCode: text('judicial_system_code'),
  degree: text('degree'),
  filingDate: timestamp('filing_date', { withTimezone: true }),
  sourceLastUpdatedAt: timestamp('source_last_updated_at', { withTimezone: true }),
  subjects: jsonb('subjects').$type<Array<{ code?: string | number | null; name?: string | null }> | null>(),
  sourceMetadata: jsonb('source_metadata').$type<Record<string, unknown> | null>(),
  source: text('source').notNull().default('DATAJUD'),
...
export const caseEvents = pgTable('case_events', {
  ...
  eventMetadata: jsonb('event_metadata').$type<Record<string, unknown> | null>(),
}, (t) => [
...
export const captureRuns = pgTable('capture_runs', {
  ...
  index('capture_runs_organization_idx').on(t.organizationId, t.startedAt),
]);

export const professionalIdentities = pgTable('professional_identities', {
  id: uuid('id').primaryKey().defaultRandom(),
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  organizationId: uuid('organization_id').notNull().references(() => organizations.id, { onDelete: 'cascade' }),
  ...
  identifiers: jsonb('identifiers').$type<Record<string, unknown> | null>(),
  metadata: jsonb('metadata').$type<Record<string, unknown> | null>(),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
}, (t) => [
  uniqueIndex('professional_identities_org_user_unique').on(t.organizationId, t.userId),
  index('professional_identities_org_idx').on(t.organizationId),
]);

export const processDiscoveryResults = pgTable('process_discovery_results', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id').notNull().references(() => organizations.id, { onDelete: 'cascade' }),
  professionalIdentityId: uuid('professional_identity_id').references(() => professionalIdentities.id, { onDelete: 'set null' }),
  runId: uuid('run_id').references(() => captureRuns.id, { onDelete: 'set null' }),
  ...
  judicialSystemCode: text('judicial_system_code'),
  classCode: text('class_code'),
  externalProcessId: text('external_process_id'),
  title: text('title'),
  area: text('area'),
  class: text('class'),
  degree: text('degree'),
  ...
  metadata: jsonb('metadata').$type<Record<string, unknown> | null>(),
  importedCaseId: uuid('imported_case_id').references(() => cases.id, { onDelete: 'set null' }),
  createdBy: uuid('created_by').references(() => users.id, { onDelete: 'set null' }),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
}, (t) => [
  index('process_discovery_results_org_status_idx').on(t.organizationId, t.status),
  index('process_discovery_results_org_number_idx').on(t.organizationId, t.processNumber),
  index('process_discovery_results_run_idx').on(t.runId),
]);
